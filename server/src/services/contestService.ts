...
export async function fetchCodeforcesContests() {
  try {
    ...
    const contests = response.data.result.filter(
      (contest: any) =>
        contest.phase === "BEFORE" || contest.phase === "FINISHED"
    );
...
// limit the number of past contests to 100, TODO Implement pagination
export async function getPastContests(
  ...
  return Contest.find(query).sort({ startTime: -1 }).limit(limit);
}

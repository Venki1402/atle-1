...
export function ContestCard({ contest, isPast = false, onContestUpdated }: ContestCardProps) {
  ...
  const handleToggleBookmark = async () => {
    ...
    console.log("venki - contest", contest);
    setIsTogglingBookmark(true);
    try {
      const updatedContest = await bookmarkContest(contest._id, !contest.isBookmarked);
  ...
  const handleUpdateSolution = async () => {
    ...
    try {
      const updatedContest = await updateSolutionUrl(contest._id, solutionUrl);
      onContestUpdated(updatedContest);
